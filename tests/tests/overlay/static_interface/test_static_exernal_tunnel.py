...
class StaticExternalTunnelTest(StaticInterfaceBaseTest.Class):
    ...
    def test_use_ipsec(self):
        '''
        Test that 'use-ipsec' is properly handled by the static tunnel interface.
        '''

        # Type verification check.
        self.assert_boolean(self.section, "use-ipsec", test_default=True)
        # Default value verification check.
        self.assert_true(self.section, "use-ipsec", expected_value=False)


    def test_ipsec_psk(self):
        '''
        Test that 'ipsec-psk' is properly handled by the static tunnel interface.
        '''
